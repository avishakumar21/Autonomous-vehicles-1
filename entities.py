...
#any thoughts on how we should pass these parameters in 
#add a parameter into tick, ca
class Entity:
    ...
    def tick(self, dt: float, dynamic_agents, potentialCollisionLeft, potentialCollisionRight): 
        if self.movable:
            speed = self.speed
            ...
            for index, i in enumerate(potentialCollisionLeft): 
                if i[0]:
                    # find the vehicle
                    for vehicle in dynamic_agents:
                        if int(vehicle.ID) == index:
                            #print(vehicle.heading)
                            if (vehicle.heading < np.pi / 0.88) and i[1] == 1:  #3.57
                                vehicle.set_control(0.15, 0.1)
                                #print(vehicle.heading)
                                i[1] = 2
                                vehicle.lane = vehicle.lane - 1
                            elif (vehicle.heading > np.pi / 0.88) and i[1] == 2: #3.57
                                vehicle.set_control(-0.15, 0.1)
                                #print(vehicle.heading)
                                i[1] = 3
                            elif vehicle.heading < np.pi and i[1] == 3:  #3.14
                                vehicle.set_control(0.00, 0.0)
                                i[1] = 1
                                potentialCollisionLeft[index][0] = False

            for index, i in enumerate(potentialCollisionRight): 
                if i[0]:
                    # find the vehicle
                    for vehicle in dynamic_agents:
                        if int(vehicle.ID) == index:
                            #print(vehicle.heading)
                            if (vehicle.heading > np.pi / 1.17) and i[1] == 1:  #2.68
                                vehicle.set_control(-0.15, 0.1)
                                #print(vehicle.heading)
                                i[1] = 2
                                vehicle.lane = vehicle.lane + 1
                            elif (vehicle.heading < np.pi / 1.17) and i[1] == 2: #2.68
                                vehicle.set_control(0.15, 0.1)
                                #print(vehicle.heading)
                                i[1] = 3
                            elif (vehicle.heading > np.pi) and i[1] == 3:  #3.14
                                vehicle.set_control(0.00, 0.0)
                                i[1] = 1
                                potentialCollisionRight[index][0] = False
    ...
    def inRangeWith(self, sensorStrength, other: Union['Point', 'Entity']) -> bool:
        if isinstance(other, Entity):
            if self.obj.distanceTo(other.obj) < sensorStrength:
                return True 
        elif isinstance(other, Point):
            if self.obj.distanceTo(other) < sensorStrength:
                return True
        else: 
            return False
        #raise NotImplementedError
